...

{
  ...
  # Use grep to find which are Celsius
  isCelsius  = grep(pattern = "C|c|Celsius|ce", x=classData$temp);

  # 2) Extract the number from every row using gsub()
  #       Make sure to keep signs and decimals
  CelsiusValues = gsub(pattern = "[^+-0123456789]", replacement="", 
                    x=classData2$temp);
  
  # 3) Create a numeric vector from the extracted numbers
  CelsiusValues2 = as.numeric(CelsiusValues);    # make the vector numeric (this is NOT automatic)
  ...
  #               C->K: K = C + 273 
  #               F->K: K = (5/9)*(F-32) + 273
  classData2$TempCelsius = CelsiusValues2; # assign vector to a new column
  
  #Convert celsius temp to fahrenheit
  TempFahrenheit = CelsiusValues2 * (9/5) + 32
  classData2$TempFahrenheit = TempFahrenheit; #assign as new column
  
  #Convert celsius temp to kelvin
  TempKelvin = CelsiusValues2 + 273
  classData2$TempKelvin = TempKelvin;#assign as new column
